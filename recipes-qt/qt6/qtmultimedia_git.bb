LICENSE = "Apache-2.0 AND BSD-3-Clause AND MPL-2.0 AND (GFDL-1.3-no-invariants-only AND (GPL-2.0-only OR GPL-3.0-only OR LGPL-3.0-only) AND (GPL-3.0-only OR LGPL-3.0-only) AND GPL-3.0-only WITH Qt-GPL-exception-1.0 OR LicenseRef-The-Qt-Company-Commercial)"
LIC_FILES_CHKSUM = " \
...
PACKAGECONFIG[pipewire] = "-DFEATURE_pipewire=ON,-DFEATURE_pipewire=OFF,pipewire"
